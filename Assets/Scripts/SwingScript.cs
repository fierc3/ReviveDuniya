...
public class SwingScript : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        swingCollider = this.GetComponent<BoxCollider>();
    }
    ...
    private void CheckTrail()
    {
        ...
        bo.size = swingCollider.size;
        bo.scale = swingCollider.transform.localScale;
        bo.rotation = swingCollider.transform.rotation;
        bo.position = swingCollider.transform.position + swingCollider.transform.TransformDirection(swingCollider.center) / 2;
                ...
                foreach (BufferObj cbo in calculated)
                {
                    ...
                    if (hits.Length > 0)
                    {
                        if (hits[0].gameObject.GetComponent<Destructible>() != null) // confere se atacou algo do cenario
                            hits[0].gameObject.GetComponent<Destructible>().SwingTrailDetected();

                        if (animController.IsAttacking() && hits[0].gameObject.GetComponentInParent<BossScript>() != null) // confere se atacou o boss
                        {
    ...
    */
    ...
    {
        if (other.gameObject.layer == 13 && animController.IsAttacking() && !sparksSource.isPlaying) // caso seja um elemento do cenario
        {
